...
関連のFake契約では未mutual/他scopeのprovider取得0、valid DMの反映と通知event、取得中のmutual失効後の反映0、明示shutdownとowner dropでaccount route停止、subscribe登録待ちとprovider取得中のshutdown取消を確認。実Irohでは別nodeの署名bindingを同一provider endpointで検証してmanifestだけを一時取得し、暗号DM frameを既存経路で復号・保存する二端末試験が、受信側に送信者ticketを事前importしない構成で成功。旧pairwise配送・通知は残している。
