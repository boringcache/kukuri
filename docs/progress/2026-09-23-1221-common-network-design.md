...
N70では既知ticket/seed外のaccount受信候補をCNの認証・同意済みtopic rendezvousで取得する。自分のrouteは各CNで更新するが、送信先route照会はschedulerがCNを1件ずつ巡回し、node別の作成順outbox cursorから最大4行だけ選ぶ。作成順索引と固定終端により、N69の2秒retry更新や連続新規挿入に影響されず古い送信先へ進む。responseは65,536byte、route最大5、各候補8、relay URL4に制限し、候補はaccount最大1,024・CN source最大8・各source最大8・45秒の未信頼窓に入れる。数値IP:port以外のaddr_hintをDNS解決せず拒否し、送信前に実QUIC署名bindingを確認する。

N70初回head `6293a573`の独立監査はblocker3でFAIL。応答後guardと候補commitのraceをtransport instance/clear epochのfenceで止め、CN別候補の上書き・全clearをsource所有とsource別cache失効へ変更し、疎なdue peekの飢餓をnode別の独立outbox cursorへ変更した。公開topic購読なしの実Iroh二端末、複数sourceの空応答と片側失効、旧fence拒否、15件の古い送信先と新規挿入、9CN選択巡回、SQLite索引・migration往復を局所contractとする。CN不使用時の著者制御state個別取得、旧pairwise受信task、起動時全件走査、既存topic refreshの全購読数依存は残し、NET-AC-2/6全体を達成としない。
