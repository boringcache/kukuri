...
| N23 `spawn_reply_target_reflection`、`reply_target_support.rs` | 未反映の返信先、台帳受付後 | permit受付後に最大4 task。docsはLocalOnly、本文remoteは一時bytesで受け、account閉鎖/private世代変更で取得取消・保存拒否 | 旧docs readerの置換はR5-B。旧scopeのtaskを新しい参加世代へ持ち込まない | NW-2/4/9、署名/LocalOnly維持 |
| N24 `MissingBodyLedger`、`hydration_limits.rs` | 欠損本文・返信先・session、初回と5/30/120秒後の最大4試行 | RAIIで失敗記録、1,024 key・key256byte。本文/返信先は同時4取得、sessionは同時2取得。表示中は所有者ごと単一期限通知で再試行し、非表示で次回通知停止。session専用futureは表示終了で取消 | 同じ表示需要を合流し、再描画・既知情報で履歴をリセットしない | NW-2/4、関連missing-body/reply/session tests |
