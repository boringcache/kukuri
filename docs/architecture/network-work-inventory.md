...
| N24 `MissingBodyLedger`、`hydration_limits.rs` | 欠損本文・返信先・session、初回と5/30/120秒後の最大4試行 | RAIIで失敗記録、1,024 key・key256byte。本文/返信先は同時4取得、sessionは同時2取得。表示中は単一期限通知で再試行し、非表示で停止 | 同じ表示需要を合流し、再描画・既知情報で履歴をリセットしない | NW-2/4、関連missing-body/reply/session tests |
