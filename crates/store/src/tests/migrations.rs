...
// を固定する。期待値は観測した現挙動の生リテラル(世代数 31 など)。
// ---------------------------------------------------------------------------
...
async fn all_generations_have_paired_down() {
    ...
    assert_eq!(
        generations.len(),
        31,
        "store migrations must cover exactly 31 generations, found versions: {:?}",
...
async fn full_migration_round_trip() {
    ...
    assert_eq!(
        ...
        31,
        "round trip must restore all 31 migration generations"
    );
