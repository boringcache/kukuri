...
接続先・同時同期数・購読の選択を統合する owner は旧#1224を統合した#1221が所有し、
[ADR 0055](0055-demand-owned-network-work.md)で設計する。本 ADR はその owner に渡す
...
2026-09-25の#1221 R5-Aでは、desktopの新しいremote record/blobと投稿projectionをaccount SQLiteの
対象索引・使用量台帳へ置き、1GiB・非利用7日・1回128件と取得中の予約を適用する。本文/添付は取得だけでは
保存せず、署名・scopeと表示権限の検証後にcacheへ確定する。保護blobが旧SDK storeに存在する場合は
二重保存せず、bookmarkの共有hash参照はcache回収から保護する。旧領域の保護移行と物理退役はR5-G/Iで行う。
成人向け添付のラベル根拠を回収した場合は、現在の対象から再検証できるまで添付を非表示にし、表示済みURLも
破棄する（2026-09-25ユーザー決定A）。
...
- 更新を案内し、bucket切替時に旧版との新着相互運用を終了する（2026-09-23ユーザー決定）。
  期限付き互換期間は設けず、旧端末が旧形式へ書いた新着を新版が自動回復する経路を作らない。
  旧保存データのうち保護対象は削除しない。
...
- 旧#1243は管理集約のためClose済みであり、実装完了を表さない。独立監査・必須CIと各受入条件は
  #1221で管理する。段階PRは `Refs #1221` とし、基盤追加だけで全条件を達成としない。
