...
N70では既知ticket/seed外のaccount受信候補をCNの認証・同意済みtopic rendezvousで取得する。自分のrouteと送信先routeは各CNの期限到来時に更新し、送信先はnode別の作成順outbox cursorから最大4行だけ選ぶ。作成順索引と固定終端により、N69の2秒retry更新や連続新規挿入に影響されず古い送信先へ進む。responseは65,536byte、route最大5、各候補8、relay URL4に制限し、候補はaccount最大1,024・CN source最大8・各source最大8・45秒の未信頼窓に入れる。数値IP:port以外のaddr_hintをDNS解決せず拒否し、送信前に実QUIC署名bindingを確認する。

N70初回head `6293a573`の独立監査はblocker3でFAIL。応答後guardと候補commitのraceをtransport instance/clear epochのfenceで止め、CN別候補の上書き・全clearをsource所有とsource別cache失効へ変更し、疎なdue peekの飢餓をnode別の独立outbox cursorへ変更した。公開topic購読なしの実Iroh二端末、複数sourceの空応答と片側失効、旧fence拒否、15件の古い送信先と新規挿入、更新前候補から作ったcacheのsource失効、SQLite索引・migration往復を局所contractとする。CN不使用時の著者制御state個別取得、旧pairwise受信task、起動時全件走査、既存topic refreshの全購読数依存は残し、NET-AC-2/6全体を達成としない。

N70二回目固定head `b7085013`の独立delta監査も2blockerでFAIL。CN選択を15秒scheduler tickで進めると、2CNの30秒rendezvous期限と位相固定して片方の送信先routeが永久に照会されない。選択処理を撤去し、期限到来した各CNがそれぞれの独立cursorを進める二CN実HTTP負例に修正した。また、検証済みcacheの元候補がCN応答で空/Yへ更新された後にCNを失効させても旧endpointが残った。cacheへ検証時のsource provenanceを記録し、更新後もそのsourceのclearで失効させる負例を追加した。初回headの12/13 CIではRust testのlock分類定数が1件古くFAILしたため、分類と合計を更新して局所testを再実行した。新固定headのdelta監査・CI成功前に解消扱いしない。

N70三回目固定head `d2a8d049`の独立delta監査では残1blocker。Aの候補からbinding照合を開始した後、Aが期限整理または8source窓のevictionでmapから消え、Aをdeactivateしても旧実装はaccountの照合revisionを進めず、失効後にA由来cacheが保存される。source clearは現在候補の有無に関係なく全accountのrevisionを進め、evict済みAの進行中照合を拒否する負例を追加した。新固定headのdelta監査・CI成功前に解消扱いしない。

N70四回目固定head `af54599a`の独立delta監査では残1blocker。evict済みsourceのclearでentry revisionを更新しても、clearはentry内番号、配送失敗invalidateは別の全体tickから番号を発行していたため、2回clear→select→clear→invalidateの有限順序で旧revisionへ戻る。clearも全体tickから各entryの新revisionを発行し、旧probeのcache保存を拒否する負例を追加した。新固定headのdelta監査・CI成功前に解消扱いしない。
